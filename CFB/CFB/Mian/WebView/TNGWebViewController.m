...
@interface TNGWebViewController ()<WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler,UINavigationControllerDelegate,UINavigationBarDelegate,TNGURLFailtureDelegate,UIGestureRecognizerDelegate>

@property (nonatomic, strong) TNGURLFailture * netWorkingFailtureView;///加载失败
...
-(void)pushCurrentSnapshotViewWithRequest:(NSURLRequest*)request{
    ...
    if ([urlStr rangeOfString:@"uland.taobao.com"].location != NSNotFound) {
        //需要拼接跳转的URL
        NSString *itemId = [urlStr componentsSeparatedByString:@"//"].lastObject;
        NSString *url = [NSString stringWithFormat:@"taobao://%@",itemId];
...
- (WKWebView *)wkWebView{
    if (!_wkWebView) {
        ...
        _wkWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, DEVICE_SCREEN_WIDTH, DEVICE_SCREEN_HEIGHT) configuration:Configuration];
...
-(UIBarButtonItem*)customBackBarItem{
    if (!_customBackBarItem) {//TJWebToolsImage.TJWebToolsImage/nav_back_icon@2x.png
        UIImage* backItemImage = [[UIImage imageNamed:@"TJWebToolsImage.bundle/nav_back_icon.png"] imageWithRenderingMode:UIImageRenderingModeAutomatic];
        UIImage* backItemHlImage = [[UIImage imageNamed:@"TJWebToolsImage.bundle/nav_back_icon.png"] imageWithRenderingMode:UIImageRenderingModeAutomatic];
